  ...
  fn: async function (inputs, exits) {
    ...
    for(let seller of sellers){
      for(let type of ptypes){
        let sellerproducts = cartproducts.filter(cp => cp.product.seller === seller && cp.product.type===type);
        if(sellerproducts.length>0){
          ...
          sellerproducts.forEach(cp=>{
            if(cp.product.type==='prize'){
              cartpoints+=cp.totalPoints;
            }else{
          ...
          try{
            if(type==='prize'){
              estado = 'Aceptado'; //Validar si el usuario tiene los puntos necesarios, de lo contrario, rechazar el pedido
              mediopago = 'PUNTOS';
            }else{
              estado = payment.data.estado;
              mediopago = inputs.data.method;
            ...
            let order = await Order.create({
              ...
              cart:cart.id,
              currentstatus:await sails.helpers.orderState(estado),
